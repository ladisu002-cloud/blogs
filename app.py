...
MODE_CONFIG = {
    ...
    "건강정보": {
        ...
        "system": (
            ...
            "글 후반부에 한 번만 CTA로 연결하되, 버튼 텍스트에는 반드시 이 글의 핵심 키워드(성분명·제품 "
            "종류 등)를 그대로 넣으세요 — 예: '인삼다당체 함유 제품 보러가기', '루테인 제품 최저가 확인하기'. "
            "클릭하면 실제로 상품 페이지로 연결된다는 걸 정직하게 드러내는 표현('보러가기', '확인하기', "
            "'비교해보기' 등)을 쓰고, '~관련 정보 더 알아보기'처럼 클릭하면 추가 정보/글이 나올 것처럼 "
            "오해하게 만드는 모호한 문구는 쓰지 마세요. 전체 톤은 광고보다 정보 전달에 무게를 두세요. "
